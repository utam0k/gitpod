...
export function copyk3sKubeconfig(options: { name: string, path: string, timeoutMS: number, slice: string }) {
    ...
    while (true) {
        ...
        if (status.code == 0) {
            exec(`kubectl --kubeconfig ${options.path} config set clusters.default.server https://${options.name}.kube.gitpod-dev.com:6443`, { silent: true, dontCheckRc: true, slice: options.slice });
            return
        }
