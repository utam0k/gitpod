...
if [[ ! -f "$TMP_DIR/gitpod-installer" ]]; then
...
# TODO: move installation of yq to the Docker image
curl -sSLo "$TMP_DIR/yq" https://github.com/mikefarah/yq/releases/download/v4.16.2/yq_linux_amd64
chmod +x "$TMP_DIR/yq"
yq="$TMP_DIR/yq"
...
until [[ "$(gcloud compute ssh gitpod-main-node --command whoami 2>/dev/null)" == "gitpod" ]]; do printf "."; sleep 1; done; echo ""
